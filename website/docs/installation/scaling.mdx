...
Scaling the worker thus greatly depends on the usage made of authentik. If not synchronization is configured and there are few one-off tasks, then the worker will practically use next to no resources. However, if the worker has to process lengthy synchronization tasks and is then backlogged with other tasks, then it needs to be scaled up.

As such, we recommend setting up some [monitoring](./monitoring.md) and observing how the worker behaves and scaling it up accordingly. You should aim to have a few tasks as possible in the worker queue.

### PostgreSQL & Redis

We recommend looking into both tools respective documentation to get pointers on how to monitor those.

The specific usage that authentik makes of those doesn't scale linearly, but rather logarithmically.
